...
angular.module("Photoramp.Services", []).factory("InstagramService", function ($q, $rootScope) {
    ...
    return {
        ...
        getUserInfo: function () {
            ...
            var deferred = $q.defer(),
                promise = authorizationResult.get("/v1/users/self").done(function (data) {
        ...
        getImages: function () {
            //create a deferred object using Angular's $q service
            var deferred = $q.defer(),
                promise = authorizationResult.get("/v1/users/self/media/recent?count=" + $rootScope.count).done(function (data) {
                    deferred.resolve(data);
                });
            return deferred.promise;
        },
        ...
        getNext: function () {
            var deferred = $q.defer(),
                promise = authorizationResult.get("/v1/users/self/media/recent?count=" + $rootScope.count + "&max_id=" + $rootScope.maxId).done(function (data) {
                    deferred.resolve(data);
                });
            return deferred.promise;
        }
